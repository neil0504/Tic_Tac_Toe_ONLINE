...
import com.neilkrishna_kabara.tic_tac_toe_online.databinding.ActivityOnlineConnectionBinding
...
//var cc: Context? = null
private const val TAG = "OnlineConnection"
class OnlineConnection : AppCompatActivity() {

    lateinit var binding: ActivityOnlineConnectionBinding

    lateinit var data: ArrayList<InvitationFragmentLobby>

    private val fragmentManager = supportFragmentManager
    private var joiningCodeFromIntent: String? = null


    @SuppressLint("ClickableViewAccessibility")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityOnlineConnectionBinding.inflate(layoutInflater)
        setContentView(binding.root)
        Log.d(TAG, "My Token -=-= ${Data.myToken}")

        // New code when user clicks Notification and code is passed in intent
        val c = intent.extras
        joiningCodeFromIntent = if (c != null){
            c.get("code") as String?
        } else{
            null
        }
...
//        cc = this
        ...
        val obj = OnlineConnectionFragment(joiningCodeFromIntent)

        fragmentManager.commit {
            replace(R.id.invitationContainer, obj, "onlineConnectionFragment")
            setReorderingAllowed(true)
            addToBackStack("onlineConnectionFragment")
...
//                }
                fragmentManager.commit {
                    addToBackStack("InvitationFragment")
                    replace(R.id.invitationContainer, obj2, "InvitationFragment")
                }
